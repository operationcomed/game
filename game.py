...
class game(ShowBase):
	...
	def __init__(self):
		...
		self.textNodePath = aspect2d.attachNewNode(gametext.text.ctlText)
		self.textNodePath.setScale(0.07)
		self.textNodePath.setPos(-1.2, 0, 0.9)

		self.textNodePath = aspect2d.attachNewNode(gametext.text.escText)
		self.textNodePath.setScale(0.07)
		self.textNodePath.setPos(-1.2, 0, -0.85)

		self.textNodePath = aspect2d.attachNewNode(gametext.text.coordinateText)
		self.textNodePath.setScale(0.07)
		self.textNodePath.setPos(-1.2, 0, 0.8)

		self.textNodePath = aspect2d.attachNewNode(gametext.text.escapeText)
		self.textNodePath.setScale(0.07)
	...
	def moveTask(self, task):
		...
		if (button_down(KB_BUTTON('a'))):
			...
			# i forgot how to oop
			#gametext.text.hideText(gametext.text)
		# jumping
		if (button_down(KB.space()) and posZ < GROUND_POS + 0.1):
			self.accelZ += 0.25
			#gametext.text.showText(gametext.text)
	...
	def coordinateTask(self, task):
		...
		# this doesn't work bcoz of the wonders of object oriented programming clashing with my 11 PM self
		if (gametext.text.SHOW_TEXT):
			gametext.text.coordinateText.setText("X:" + str(posX) + " Y:" + str(posY) + "  Z:" + str(posZ))
		else:
			gametext.text.coordinateText.setText("")

		if (abs(posX) > 80 or abs(posY) > 30):
			gametext.text.escapeText.setText("YOU HAVE ESCAPED\nfrom msu!")
			mainmenu.background.destroy()
		return Task.cont